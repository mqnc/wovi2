...
public:
	...
	unique_ptr<collision::Shape> collisionShape;
	unique_ptr<collision::Shape> visualShape;

	PartDescriptor() = default;

	PartDescriptor(
		string jointId,
		string linkId,
		JointType type,
		const btTransform& origin,
		const btVector3& axis,
		const array<double, 2>& limits,
		string parentLinkId,
		unique_ptr<collision::Shape> collisionShape,
		unique_ptr<collision::Shape> visualShape
		)
		: jointId(std::move(jointId)),
		linkId(std::move(linkId)),
		type(type),
		origin(origin),
		axis(axis),
		limits(limits),
		parentLinkId(std::move(parentLinkId)),
		collisionShape(std::move(collisionShape)),
		visualShape(std::move(visualShape))
	{}

	PartDescriptor(const PartDescriptor& other)
		...
		axis(other.axis),
		limits(other.limits),
		parentLinkId(other.parentLinkId),
		collisionShape(other.collisionShape ? other.collisionShape->clone() : nullptr),
		visualShape(other.visualShape ? other.visualShape->clone() : nullptr)
	{}
};
